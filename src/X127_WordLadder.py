...
class Solution(object):
    def ladderLength(self, beginWord, endWord, wordList):
        ...
        for word in words:
            for i in range(len(word)):
                tempWord = word[:i] + '_' + word[(i+1):]
                if tempWord not in helperDict:
                    helperDict[tempWord] = []
                helperDict[tempWord].append(word)

        curLen = 1
        curWords = deque([beginWord])

        while curWords:
            qz = len(curWords)
            for _ in range(qz):
                word = curWords.popleft()
                for i in range(len(word)):
                    tempWord = word[:i] + '_' + word[(i+1):]
                    for newWord in helperDict.get(tempWord):
                        if newWord == endWord:
                            return curLen + 1
                        if newWord in words:
                            curWords.append(newWord)
                            words.remove(newWord)
            curLen += 1

        return 0

    def ladderLengthAccepted2(self, beginWord, endWord, wordList):
        """
        :type beginWord: str
        :type endWord: str
        :type wordList: List[str]
    ...
    def ladderLengthAccepted1(self, beginWord, endWord, wordList):
        """
        :type beginWord: str
        :type endWord: str
        :type wordList: List[str]
    ...
    def ladderLength4(self, beginWord, endWord, wordList):
        ...
        return self.firstShortestPath2(wordList, endIdx, startIdx)


    def firstShortestPath2(self, wordList, endIdx, startIdx):
        ...
        while cur:
            ...
            for curIdx in cur:
                ...
                for i, word in enumerate(wordList):
                    if not visited[i] and self.diffByOne(curWord, word):
                        nxt.append(i)
                        visited[i] = True
    ...
    def ladderLength3(self, beginWord, endWord, wordList):
        ...
        return self.firstShortestPath(wordList, endIdx, startIdx)


    def firstShortestPath(self, wordList, endIdx, startIdx):
        visited = [False] * len(wordList)
        q = deque()
        q.append(endIdx)
        curLen = 1
    ...
    def firstShortPath(self, g, endIdx, startIdx):
        visited = [False] * len(g)
        q = deque()
        q.append(endIdx)
        curLen = 1
        ...
        while q:
            ...
            for _ in range(qz):
                curIdx = q.popleft()
                if curIdx in startIdx:
                    return curLen

                for neigIdx in g[curIdx]:
                    if not visited[neigIdx]:
                        q.append(neigIdx)
    ...
    def ladderLength2(self, beginWord, endWord, wordList):
        ...
        for i, word in enumerate(wordList):
            ...
            if self.diffByOne(word, beginWord):
                startIdx.append(i)

        if not startIdx or endIdx == -1:
            return 0

        g = self.getGraph(wordList)
        lengths = self.shortPath(g, endIdx)

        minLen = len(wordList) + 1
        for idx in startIdx:
            curLen = lengths[idx]
            if curLen != -1:
